...
4. show the new branch explanation
5. Do the work
6. Open a PR with `Closes #54` in the body — GitHub closes the issue automatically on merge
...
| Version releases | release-please (`googleapis/release-please-action@v4`) |

---

## Version tagging

This project uses **release-please** (`googleapis/release-please-action@v4`) to manage all git tags and GitHub Releases automatically.

### How it works

1. A PR merges to `main` — the squash commit message is the PR title (e.g. `feat: add RDS module`)
2. The release-please GitHub Actions workflow reads that commit and determines the version bump:
   - `feat:` → **minor** bump (1.0.0 → 1.1.0)
   - `fix:` → **patch** bump (1.0.0 → 1.0.1)
   - `feat!:` or `BREAKING CHANGE:` in the commit footer → **major** bump (1.0.0 → 2.0.0)
   - `chore:`, `docs:`, `refactor:`, `infra:` → **no bump** (accumulated silently)
3. release-please opens or updates a single "Release PR" titled `chore: release X.Y.Z` containing:
   - An updated `CHANGELOG.md` listing all commits since the last release
   - An updated `VERSION` file with the new version number
4. When **you merge the Release PR**, release-please creates the git tag and the GitHub Release automatically

### Key files (created when we implement this)

| File | Purpose |
|---|---|
| `.github/workflows/release-please.yml` | The workflow that runs on every push to `main` |
| `release-please-config.json` | Tells release-please the release type (`simple`) and which files to update |
| `.release-please-manifest.json` | Tracks the current version — release-please updates this automatically |
| `CHANGELOG.md` | Auto-generated by release-please — never edit by hand |
| `VERSION` | Plain text file containing the current version (e.g. `1.2.0`) |

### Rules

- **Never create git tags manually** — release-please owns all version tags. Manual tags will confuse it.
- **Never rename the Release PR** — its title `chore: release X.Y.Z` is how release-please identifies it.
- **Never edit CHANGELOG.md by hand** — it is fully managed by release-please.
- Merging the Release PR = publishing a release. Only merge it when you are ready to cut a version.
- You can let the Release PR sit open while more features land — it updates itself automatically.
...
- Do not create git tags manually — release-please owns all version tags
- Do not edit CHANGELOG.md by hand — it is generated by release-please
- Do not rename the release-please Release PR — its title must stay `chore: release X.Y.Z`
